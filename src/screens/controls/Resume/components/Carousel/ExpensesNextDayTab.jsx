...
const ExpensesNextDayTab = ({ empresaId, selectedDate }) => {
    ...
    try {
      if (empresaId && selectedDate) {
        ...
        if (response.status === 200 && response.data.status === 'success') {
          const itemData = response.data.data.find((item) => item.item === 'Despesas à Vencer Próximo dia Útil');
          setData(itemData);
        } else {
          setData(null);
        }
...
export default ExpensesNextDayTab;
