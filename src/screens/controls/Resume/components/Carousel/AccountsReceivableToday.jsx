...
const AccountsReceivableToday  = ({ empresaId, selectedDate }) => {
    ...
    try {
      if (empresaId && selectedDate) {
        ...
        if (response.status === 200 && response.data.status === 'success') {
          const itemData = response.data.data.find((item) => item.item === 'Contas a Receber à Vencer Hoje');
          setData(itemData);
        } else {
          setData(null);
        }
...
export default AccountsReceivableToday;
